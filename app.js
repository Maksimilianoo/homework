    ...
    get summary() {
        ...
        if (mediumMark > 9 && mediumVisit > 0.9) {
            return `Середня оцінка: ${mediumMark.toFixed(
                2
            )}. Середнє відвідування: ${mediumVisit.toFixed(2)}.Так тримати!`;
        } else if (mediumMark > 9 || mediumVisit > 0.9) {
            return `Середня оцінка: ${mediumMark.toFixed(
                2
            )}. Середнє відвідування: ${mediumVisit.toFixed(
                2
            )}. Можна постаратись краще.`;
        } else {
            return `Середня оцінка: ${mediumMark.toFixed(
                2
            )}. Середнє відвідування: ${mediumVisit.toFixed(
                2
            )}. Так ти нічому не навчишся.`;
        }
    }
}
...
const student = new Student("Наталія", "Діма", 1998);
student.setMark(11);
student.setMark(12);
student.present();
student.present();
...
const teacher = new Teacher("Богдана", "Богдан", 2003);
teacher.addGroup({ name: "Математика", active: true });
teacher.addGroup({ name: "Англійська", active: false });
teacher.changeStatus({ name: "Математика", active: true });
